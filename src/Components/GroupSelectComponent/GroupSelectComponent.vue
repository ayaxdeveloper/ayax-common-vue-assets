<template>
  <v-autocomplete v-model="innerValue" :multiple="multiple" :items="groupedItems" v-bind="$attrs">
    <template :slot="useSelectionSlot ? 'selection': ''" slot-scope="data">
    ...
    <template slot="item" slot-scope="data">
      <template>
        ...
        <template v-else>
          ...
          >
            <v-icon :color="data.item.selected ?'primary' : ''">{{ checkboxState(data.item) }}</v-icon>
          </v-list-tile-action>
          <v-list-tile-content
            @click="clickHandler(data.item)"
    ...
    if (this.groups.size > 0) {
      this.groups.forEach(group => {
        ...
        this.groupedItems = this.groupedItems.concat(
          this.items.filter(item => item.group && item.group === group)
        );
      });
      this.groupedItems = this.groupedItems.concat(
